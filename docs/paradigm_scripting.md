...
The name of the script item is sent as an event marker in the LSL stream _yaga_ when the item is triggered. This allows recording programs to save the paradigm state together with the LSL data streams (see Section [Integration with LSL](integration_with_lsl.md)).

Note that the execution of script items is synchronised to the screen refresh rate (often 60 Hz).

### Actions

The actions parameter of a **ScriptItem** expects a list of methods which are called when the action item is triggered. The available actions depend on the presentation object (see Sections [Supported Graphical Objects](paradigm_scripting.md#supported-graphical-objects) & [Auditory Objects](paradigm_scripting.md#auditory-objects) for details on the actions).
...
A script item can wait for trigger signals generated by graphical objects. To trigger a script item with a signal, set *wait_for_signal* to the respective signal. See Section [Supported Graphical Objects](paradigm_scripting.md#supported-graphical-objects) for possible trigger signals. For example:
...
Graphical objects are objects shown on the computer screen, such as text or feedback bars. They may be controllable with LSL streams (see Section [Control of Graphical Objects](paradigm_scripting.md#control-of-graphical-objects)) and can feature animations.
...
To also control the vertical target position with an LSL stream, set *target_online_control* to true (alternatively: use the action **updateTargetValueFromLSLStream**), and specify two control channels. For example:
...
| target_online_control | bool              | * control the vertical target position with an LSL stream <br> * the stream can be the same or a different stream than the stream used to control the fill level (in the later case use **controlStateWithLSLStream<u>s</u>** with the specified second stream being the target stream) <br> * if _true_, a second control channel for the target position must be specified when calling **controlStateWithLSLStream** or **controlStateWithLSLStreams** <br> * if _false_, the target position can still be updated with the action **updateTargetValueFromLSLStream** |
...
Displays a feedback bar (see [Bar Class](paradigm_scripting.md#bar-class)) with a ramp animation of the target. The target will transit through the following phases:
...
Displays a feedback bar (see [Bar Class](paradigm_scripting.md#bar-class)) with a sinus oscillation animation of the target. The target will transit through the following phases:
...
To set up the control of the arrow’s endpoint, call the method **controlStateWithLSLStream** and specify two control channels. For example:
...
To also control the vertical target position with an LSL stream, set _target_online_control_ to _true_ (alternatively: use the action **updateTargetValueFromLSLStream**), and specify three control channels. For example:
...
| target_online_control | bool              | * control the vertical target position with an LSL stream <br> * the stream can be the same or a different stream than the stream used to control the endpoint of the arrow (in the later case use **controlStateWithLSLStream<u>s</u>** with the specified second stream being the target stream) <br> * if _true_, a third control channel for the target position must be specified when calling **controlStateWithLSLStream** or **controlStateWithLSLStreams** <br> * if _false_, the target position can still be updated with the action **updateTargetValueFromLSLStream** |
...
Displays a feedback arrow (see [Arrow Class](paradigm_scripting.md#arrow-class)) with a ramp animation of the target disk. The target will transit through the following phases:
...
Auditory objects generate sounds. They may be controllable with LSL streams (see [Control of Auditory Objects](paradigm_scripting.md#control-of-auditory-objects)).
...
| beep_channels         | string          | list of output channels (“both”, “left”, or “right”); must have the same length as number of LSL channels                                                                                                            |
...
| dynamic_frq_factor    | integer         | the actual sound frequency is the base frequency (beep_frequencies) plus the instantaneous firing rate multiplied by this dynamic frequency factor                                                                   |
...
| controlColorWithLSLStream | lsl_stream_name, channel, aggregation_mode, neg_color, pos_color, neutral_color | string, integer, string, string, string, string | interpolate between colors using one channel from a single LSL stream: <br> when the value of the LSL channel is in the interval \[-1, 0): interpolate between the negative and neutral color <br> when the value of the LSL channel is in the interval \[0, 1\]: interpolate between the neutral and positive color <br> colors must be specified by their name (string) |
| controlStateWithLSLStream | lsl_stream_name, channels, aggregation_mode                                     | string, list of integers, string                | state updates are specific to the graphical object (see TODO)                                                                                                                                                                                                                                                                                                                   |

-   the *channels* parameter must be a list comprising two channel indices

-   the *channel* parameter for *controlColorWithLSLStream* must be the index of a single channel

-   the *channels* parameter for *controlStateWithLSLStream* must be a list with one or more channel indices (the number of channels depends on the actual graphical object)

-   for an explanation of the *aggregation_mode* parameter see Section TODO.

-   **note that indexing starts with a 0 in Python**
...
| controlStateWithLSLStreams | lsl_stream_names, channels, aggregation_mode | list of strings, list of integers, string | so far only supported by Bar and Arrow, see their description for details             |
...
| method                | parameters                               | value types                              | description                                               |
|-------------------|--------------|--------|-------------------------------|
| controlWithLSLStream  | stream name, channels, aggregation_mode  | string, list of integers, string         | the control is specific to the auditory object (see TODO) |
| controlWithLSLStreams | stream names, channels, aggregation_mode | list of string, list of integers, string | so far not supported by any auditory object               |
...
-   When the sampling frequency of an LSL stream is lower than the screen refresh rate, the LSL stream is up-sampled to the screen frequency using a sample & hold strategy.

-   When the sampling frequency of an LSL stream is higher than the screen frequency, the downsampling strategy specified by *aggregation_mode* is used.
...
LSL streams can be processed when used to control parameters of graphical or auditory objects. The YAGA signal processing methods are applied to the *original* LSL stream, i.e., before any re-sampling to the screen refresh rate (see Aggregation Mode TODO).

To use signal processing on LSL streams, import the module signal_processing:
...
| cutoff_frqs | double or list of doubles | critical frequency or frequencies; a scalar for a lowpass or highpass filter; a 2-element list for a bandpass or bandstop filter |
...
| parameter | value type | description                                                                                        |
|----------------------|-------------|--------------------------------------|
| in_val1   | double     | linearly map signals from the interval \[in_val1, in_val2\] to the interval \[out_val1, out_val2\] |
| in_val2   | double     |                                                                                                    |
| out_val1  | double     |                                                                                                    |
| out_val2  | double     |                                                                                                    |
...
Note: The signal processing object expects 2 channels, but only the first channel is evaluated.
