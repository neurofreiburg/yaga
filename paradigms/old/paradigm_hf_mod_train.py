...
class Paradigm(ParadigmBase):
    ...
    def __init__(self, paradigm_variables):
        ...
        for trial_idx in range(n_trials):
            ...
            if trial_class == 1:
                rate_modulation_cue = left_cue
            elif trial_class == 2:
                rate_modulation_cue = right_cue

            trial_script_items.extend([ScriptItem(name='class' + str(trial_class), time=0, time_type='rel', rel_name='trial_start', actions=[cross.activate, force_target.activate, direct_feedback.activate]),
                                        ScriptItem(name='rate_cue_on', time=cue_delay, time_type='rel', rel_name='trial_start', actions=[rate_modulation_cue.activate]),
                                        ScriptItem(name='rate_cue_off', time=cue_delay + 2, time_type='rel', rel_name='trial_start', actions=[rate_modulation_cue.deactivate]),
