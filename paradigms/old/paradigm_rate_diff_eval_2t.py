...
class Paradigm(ParadigmBase):
    ...
    def __init__(self, paradigm_variables):
        ...
        # class3 = self.registerObject(GO.Text("\u2196", pos_x=0, pos_y=0, scale_x=0.7, scale_y=0.7, color='white')) # upper left
        # class4 = self.registerObject(GO.Text("\u2197", pos_x=0, pos_y=0, scale_x=0.7, scale_y=0.7, color='white')) # upper right

        cue_y_pos = (target_1_y_pos + target_2_y_pos)/2 - 0.25/4 - 0.3
        up_cue = self.registerObject(GO.Text("\u22C0", pos_x=0, pos_y=cue_y_pos, scale_x=0.25, scale_y=0.25, color='white')) # arrow up
        down_cue = self.registerObject(GO.Text("\u22C1", pos_x=0, pos_y=cue_y_pos, scale_x=0.25, scale_y=0.25, color='white')) # arrow down
        ...
        for trial_idx in range(n_trials):
            ...
            trial_script_items.extend([ScriptItem(name='class' + str(trial_class), time=0, time_type='rel', rel_name='trial_start', actions=[cross.activate, force_target_1.activate, force_target_2.activate, direct_feedback.activate, force_cue.activate]),
                                        ScriptItem(name='rate_cue_on', time=7, time_type='rel', rel_name='trial_start', actions=[force_cue.deactivate, rate_cue.activate]),
                                        ScriptItem(name='rate_cue_off', time=9, time_type='rel', rel_name='trial_start', actions=[rate_cue.deactivate]),
                                        ScriptItem(name='trial_end', time=trial_duration, time_type='rel', rel_name='trial_start', actions=[cross.deactivate, force_target_1.deactivate, force_target_2.deactivate, direct_feedback.deactivate])])
