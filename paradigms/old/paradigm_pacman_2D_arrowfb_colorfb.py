...
class Paradigm(ParadigmBase):
    ...
    def __init__(self, paradigm_variables):
        ...
        arrow_angle = -18
        arrow_target_force = 0.10
        arrow_max_force = 0.15
        arrow_filter_frq = 5
        ...
        arrow = self.registerObject(GO.Arrow(pos_x=-1, pos_y=-0.9, depth=5, angle=arrow_angle, arrow_length=0.6, line_width=0.01, head_size=0.05, high_value=arrow_max_force, target_size=0.1, target_color='cyan', target_value=arrow_target_force))

        # pacman.controlStateWithLSLStreams(['MouseControllerStream', 'quattrocento'], channels=[1, 64])
        # pacman.controlStateWithLSLStreams(['quattrocento', 'quattrocento'], channels=[64, 64])
        ...
        butter_arrow = SP.ButterFilter(4, arrow_filter_frq)
        # arrow.controlStateWithLSLStream('MouseControllerStream', channels=[1, 0])
        # arrow.addSignalProcessingToLSLStream(butter_arrow, channels=[0, 1])
        ...
        arrow.addSignalProcessingToLSLStream(butter_arrow, channels=[64, 65])
        ...
        # force
        # scaler_mu = SP.Scaler(scale=20, post_offset=-1)
        # pacman.addSignalProcessingToLSLStream(max_normalization, channels=[64, 65], lsl_stream_name='quattrocento')
        # pacman.addSignalProcessingToLSLStream(scaler_mu, channels=[64], lsl_stream_name='quattrocento')
        # pacman.addSignalProcessingToLSLStream(butter_mu_vis, channels=[64], lsl_stream_name='quattrocento')
        # mouse
