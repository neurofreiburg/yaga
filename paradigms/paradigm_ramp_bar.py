...
class Paradigm(ParadigmBase):
    ...
    def __init__(self, paradigm_variables):
        ...
        n_trials = 1
        pre_paradigm_interval = 1
        post_paradigm_interval = 1
        ...
        bar.controlStateWithLSLStream('MouseControllerStream', channels=[0])
        # bar.controlColorWithLSLStream('MouseControllerStream', channel=1)
        ...
        for trial_idx in range(n_trials):
            ...
            trial_script_items.extend([ScriptItem(name='info_text', time=0, time_type='rel', rel_name='trial_start', actions=[info_text.activate]),
                                        ScriptItem(name='traffic_light_red', time=1, time_type='rel', rel_name='trial_start', actions=[info_text.deactivate, traffic_light_red.activate]),
                                        ScriptItem(name='traffic_light_yellow', time=2, time_type='rel', rel_name='trial_start', actions=[traffic_light_red.deactivate, traffic_light_yellow.activate]),
                                        ScriptItem(name='traffic_light_greeb', time=3, time_type='rel', rel_name='trial_start', actions=[traffic_light_yellow.deactivate, traffic_light_green.activate]),
                                        ScriptItem(name='start_ramp', time=4, time_type='rel', rel_name='trial_start', actions=[traffic_light_green.deactivate, bar.activate, bar.startAnimation]),
