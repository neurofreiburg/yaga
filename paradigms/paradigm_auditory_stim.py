...
class Paradigm(ParadigmBase):
    ...
    def __init__(self, paradigm_variables):
        ...
        for trial_idx in range(n_trials):
            # trial start
            if trial_idx == 0:
                self.script.append(ScriptItem(name='trial_start', time=pre_paradigm_interval, actions=[attention_beep.activate, info_text.activate]))
            else:
                self.script.append(ScriptItem(name='trial_start', time=np.random.uniform(inter_trial_interval_min, inter_trial_interval_max), time_type='rel', rel_name='trial_end', actions=[attention_beep.activate, info_text.activate]))
            ...
            for stimulus_idx in range(n_stimuli_per_trial):
                ...
                self.script.append(ScriptItem(name='stimulus', time=current_stimulus_time, time_type='rel', rel_name='show_bar', actions=[stimulus.activate]))
