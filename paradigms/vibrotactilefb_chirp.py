...
class Paradigm(ParadigmBase):
    ...
    def __init__(self, paradigm_variables):
        ...
        pacman_item_generation_frequency = 50 # for 1 Hz
        ...
        pacman_item_generation_frequency = 50

        info_text = self.registerObject(GO.Text('prepare for force tracking', scale_x=0.1, scale_y=0.1, color='white'))
        pacman = self.registerObject(Pacman(item_speed=initial_distance_to_pacman/pacman_delay, item_generator='chirp', item_generation_frequency=pacman_item_generation_frequency, frequency=[chirp_start_frq, chirp_end_frq], amplitude=pacman_amplitude, phase_duration=chirp_duration))
        ...
        map_y = SP.LinearMap(chirp_min_force, chirp_max_force, -pacman_amplitude, pacman_amplitude)
        ...
        for trial_idx in range(n_trials):
            ...
            self.script.append(ScriptItem(name='generator_stop', time=chirp_duration, time_type='rel', rel_name='pacman_start', actions=[pacman.stop]))
